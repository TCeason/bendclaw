...
/// Return the root sessions directory: `~/.bend-agent/sessions`.
fn get_sessions_dir() -> PathBuf {
    let home = std::env::var("HOME")
        .or_else(|_| std::env::var("USERPROFILE"))
        .unwrap_or_else(|_| "/tmp".to_string());
    Path::new(&home).join(".bend-agent").join("sessions")
}
...
/// Persist a session to `~/.bend-agent/sessions/{id}/transcript.json`.
