...
/// Levels absent from a map fall through to the protocol default, so only the
/// opt-in tiers (`xhigh`/`max`) and explicit exclusions need entries.
const GPT_LEVELS: &[(ThinkingLevel, Option<&str>)] = &[(ThinkingLevel::Xhigh, Some("xhigh"))];
...
const PROFILES: &[(&str, ModelProfile)] = &[
    ("gpt-5.4",       ModelProfile { context_window: 272_000, max_tokens: 128_000, thinking_levels: GPT_LEVELS, first_party_responses_thinking_levels: OFF_NONE, remote_compaction: true, ..BASE }),
...
pub(super) fn fallback(id: &str) -> Option<ModelProfile> {
    if id.starts_with("gpt-") || id.starts_with("codex-") {
            ...
            .any(|family| id.contains(family))
        {
            ...
            // Older GPT-5 / codex families expose no xhigh or max tier.
            &[]
        };
