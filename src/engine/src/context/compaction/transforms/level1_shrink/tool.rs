...
pub fn run(messages: Vec<AgentMessage>, ctx: &PhaseContext, current_tokens: usize) -> PhaseResult {
    ...
    let recent_boundary = len.saturating_sub(ctx.bounds.keep_recent);

    let oversize_token_threshold = ctx
        .policy
        .oversize_abs_tokens
        .max((ctx.budget.max_tokens as f64 * ctx.policy.oversize_budget_ratio) as usize);
    ...
    for (idx, msg) in messages.into_iter().enumerate() {
        ...
        let is_pinned = idx < ctx.bounds.keep_first;
        ...
        if let AgentMessage::Llm(Message::User { content, timestamp }) = &msg {
            ...
            match action {
                UserAction::TruncateOversized => {
                    ...
                    let max_lines = ctx.shrink.tool_output_max_lines;
        ...
        }) = msg
        {
            ...
            match decision {
                ToolShrinkDecision::OversizeCap => {
                    let max_lines = tp.oversize_max_lines.min(ctx.shrink.tool_output_max_lines);
                ...
                ToolShrinkDecision::NormalTrunc => {
                    let max_lines = tp.normal_max_lines.min(ctx.shrink.tool_output_max_lines);
                    let truncated = truncate_content(
                        &content,
                        &tool_name,
                        &tool_call_id,
                    ...
                    if let Some(stripped) = strip_images_under_severe_pressure(
                        &content,
                        running_tokens,
                        ctx.budget.max_tokens,
                    ) {
